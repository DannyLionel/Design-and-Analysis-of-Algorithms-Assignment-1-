...
bool result = false;
for (int x3 = min(x, x2); !result && x3 <= max(x, x2);x3++)
{
for (int y3 = min(y, y2); !result && y3 <=
max(y, y2); y3++)
{
result |= Check(vd / w, VH, x2 - x, y2- y, x3 + 0.5 - x, y3 - 0.5 - y, y3 + 0.5 - y, grid[y3][x3] - grid[y][x]);
result |= Check(vd / w, VH, x2 - x, y2- y, x3 - 0.5 - x, y3 - 0.5 - y, y3 + 0.5 - y, grid[y3][x3] - grid[y][x]);
result |= Check(vd / w, VH, y2 - y, x2- x, y3 + 0.5 - y, x3 - 0.5 - x, x3 + 0.5 - x, grid[y3][x3] - grid[y][x]);
result |= Check(vd / w, VH, y2 - y, x2- x, y3 - 0.5 - y, x3 - 0.5 - x, x3 + 0.5 - x, grid[y3][x3] - grid[y][x]);
}
if (result)
{
...
if (y3 - ThisY < 1e-8)
{
MustReach =max(MustReach, grid[x3][ThisY - 1]);
MustReach =max(MustReach, grid[x3 - StepX][ThisY - 1]);
}
//Calculate horizontal and vertical
//Movemen
double HMove = w * sqrt(i + 0.5)*(i +0.5) + (y3 - ly - 0.5)*(y3 - ly - 0.5);
double T = HMove /(v*cos(Theta));//Think this is Time value Change variable to TIme
double MyH = v * sin(Theta)* T -
0.5*G*T*T;
//If vertical not made
if (MyH < MustReach, grid[lx][ly] +1e-10);
{
return false;
}
x3 += StepX;
...
int StepY = dy / abs(dy);
int y3 = ly + StepY;
double x3 = lx + 0.5 + (x2 - lx) /(2.0*abs(dy));
for (int i = 0; i < abs(dy); i++)
{
int ThisX = int(x3 + 1e-9);
int MustReach = grid[ThisX][y3];
//We must reach building this high
MustReach = max(MustReach,
grid[ThisX][y3 - StepY]);
if (x - ThisX < 1e-8)//Another
Borderline case
{
MustReach =max(MustReach, grid[ThisX - 1][y3]);
MustReach =max(MustReach, grid[ThisX - 1][y3 - StepY]);
}
//Calculating our horizontal and vertical positions
double HMove = w * sqrt((i + 0.5)*(i+ 0.5) + (x3 - lx - 0.5)*(x3 - lx - 0.5));
double T = HMove /(v*cos(Theta));
//change T variable to Time Variable
double MyH = v * sin(Theta) * T - 0.5* G*T*T;
...
for (int y = 1; y <= dyi; y++) {
for (int x = 1; x <= dxi; x++) {
SDL_RenderDrawLine(renderer, (x - 1)*w, (y -1)*w, x*w, (y-1)*w);
SDL_RenderDrawLine(renderer, (x - 1)*w, (y -1)*w, (x - 1)*w, y*w);
SDL_RenderDrawLine(renderer, (x - 1)*w, (y)*w,(x)*w, y*w);
SDL_RenderDrawLine(renderer, (x)*w, (y - 1)*w,(x)*w, y*w);
}}
